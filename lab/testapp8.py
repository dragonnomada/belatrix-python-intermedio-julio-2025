...
class TodoModel(QObject):
    ...
    def actualizarTodo(self, id, titulo, verificado):
        print("Actualizando Todo {id}...")

        cursor = self.conn.cursor()

        cursor.execute("UPDATE todos SET titulo=?, verificado=? WHERE id=?", (titulo, verificado, id))
...
class TodoView(QObject):
    ...
    signalActualizarTodo = Signal(int, str, bool)
    ...
    def actualizarTodoItem(self, todoItem):
        ...
        self.celdaActiva = None
        self.celdaValor = None
        ...
        self.signalActualizarTodo.emit(int(todoId), todoTitulo, todoVerificado == "✅")
...
class TodoController(QObject):
    ...
    def inicializar(self):
        ...
        self.todoView.signalActualizarTodo.connect(self.todoModel.actualizarTodo)
